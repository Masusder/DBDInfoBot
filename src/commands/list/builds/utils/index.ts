...
export async function createEmbed(
    ...
    const embed = new EmbedBuilder()
        .setTitle(`${t('list_command.builds_subcommand.builds_list', locale, ELocaleNamespace.Messages)} - ${t('list_command.builds_subcommand.builds_list_page', locale, ELocaleNamespace.Messages, {current_page: currentPage.toString(), total_pages: (totalPages + 1).toString()})}`)
        .setColor(role === 'Survivor' ? ThemeColors.SURVIVOR : ThemeColors.KILLER)
        .setDescription(t('list_command.builds_subcommand.builds_matching_filters', locale, ELocaleNamespace.Messages))
        .setTimestamp()
        .setFooter({ text: t('list_command.builds_subcommand.builds_list', locale, ELocaleNamespace.Messages) })
    ...
    for (const build of builds) {
        ...
        const buildTitle = `${index + 1}. ${build.title} • ${t('list_command.builds_subcommand.created_by', locale, ELocaleNamespace.Messages)} ${build.username}`;
        ...
        const perksList = resolvedPerkData
            ...
            .join(' ') || t('list_command.builds_subcommand.any_perks', locale, ELocaleNamespace.Messages);
...
export function createStringMenu(builds: IBuild[], locale: Locale) {
    const selectMenu = new StringSelectMenuBuilder()
        ...
        .setPlaceholder(t('list_command.builds_subcommand.select_for_details', locale, ELocaleNamespace.Messages))
        .addOptions(
            builds.map((build: IBuild, index: number) => ({
                label: `${index + 1}. ${build.title}`,
                description: `${t('list_command.builds_subcommand.created_by', locale, ELocaleNamespace.Messages)} ${build.username}`,
                value: build.buildId
            }))
        );
...
export function createLinkButton(locale: Locale) {
    const button = new ButtonBuilder()
        .setLabel(t('list_command.builds_subcommand.create_your_own_build', locale, ELocaleNamespace.Messages))
